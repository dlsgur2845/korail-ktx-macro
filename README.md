...
코레일 일반 예매 웹에서 원하는 KTX 좌석을 감시하고, 예매 또는 예약대기 신청을 보조하는 Chrome 확장 프로그램입니다. **결제는 직접 진행합니다.** 명절 페이지에서는 직접 선택한 버튼을 지정 시각에 한 번 누르는 별도 기능을 제공합니다.

현재 버전 **2.0.5** · [업데이트 내역](RELEASE_NOTES.md)
...
항상 웹 조회 인원을 **1명**으로 설정하세요. 예약 또는 예약대기 접수가 완료되면 정지하고 알립니다. 여러 명 연속 신청은 지원하지 않습니다. 감시 시작을 누르면 이전 접수 기록과 관계없이 새 감시를 시작합니다.
...
Chrome 화면과 모의 테스트로 일부 동작을 검증했습니다. 실제 매진 경쟁 상황의 좌석 확보, 모든 예약 화면 형식은 보장하지 않습니다. 로그 삭제는 Windows·Mac Chrome 공통 API로 구현했지만 두 OS의 실제 브라우저에서 모두 검증한 것은 아닙니다. 버전별 변경과 검증 내역은 [릴리스 노트](RELEASE_NOTES.md)를 참고하세요.

## 화면 꺼짐·자동 절전 방지

실행 중에는 Chrome `power` 권한으로 화면과 시스템을 켜둡니다. 일반 감시와 명절 정시 클릭 대기에 적용합니다. 중지·완료·탭 종료 시 해제하며, 응답이 끊긴 탭은 약 90~120초 후 해제합니다. 업데이트 후 확장 프로그램을 다시 로드하고 요청되는 권한을 확인해주세요.

배터리 소모는 늘어납니다. 덮개 닫기·수동 잠자기·배터리 부족·Chrome 종료를 막거나 로그인 상태를 유지하는 기능은 아닙니다. Windows/macOS 실제 절전 동작은 추가 확인이 필요합니다.
